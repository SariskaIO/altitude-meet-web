...
import {GENERATE_TOKEN_URL, GET_PRESIGNED_URL, ENTER_FULL_SCREEN_MODE, LIVE_STREAMING_START_URL, LIVE_STREAMING_STOP_URL, ALTITUDE_MESSAGE_URL} from "../constants";
...
export async function getToken(profile, name, id, avatarColor) {
    const body = {
        method: "POST",
        headers: {
...
export async function sendApiMessage(meetUrl) {
    const body = {
        method: "POST",
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Basic bWVzc2FnZXM6eHh4eHh4eHh4eA==`
        },
        body: JSON.stringify({
            type: "message",
            id: "8jOJFMqIq9t5hxvKgCmJnK-a|0000007",
            ...
            channelData: {
                clientActivityID: "1592838678818idjvh6pyloj",
                clientTimestamp: (new Date()).toISOString(),
                attributes: [ { name: "ct_transferto", value: meetUrl } ]
            }
        })
    };
    ...
    try {
        const response = await fetch(ALTITUDE_MESSAGE_URL, body);
        if (response.ok) {
            const json = await response.json();
